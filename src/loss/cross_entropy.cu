...
float CrossEntropyLoss::loss(Tensor *predict, Tensor *target)
{
    int num_sms;
    ...
    softmax_loss_kernel<<< num_blocks, BLOCK_DIM_1D, BLOCK_DIM_1D * sizeof(float), 0 >>>(d_loss_,
    		predict->get_device_ptr().get(), target->get_device_ptr().get(), d_workspace_, batch_size, num_outputs);
